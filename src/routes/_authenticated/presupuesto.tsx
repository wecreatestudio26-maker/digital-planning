  ...
  return (
    <div className="space-y-6">
      <div className="flex items-end justify-between gap-4">
        <div>
          <h2 className="text-2xl font-semibold tracking-tight">{t("budget.title")}</h2>
      ...
      {overbudget.length > 0 && (
        <div className="rounded-lg border border-destructive/40 bg-destructive/10 p-4 flex gap-3">
          ...
          <div className="text-sm">
            <p className="font-medium text-destructive">{t("budget.overbudget_title", { count: overbudget.length })}</p>
      ...
      <div className="grid gap-4 grid-cols-2 lg:grid-cols-4">
        {[
          { label: t("budget.planned"), value: fmt(planned), icon: Wallet },
          { label: t("budget.actual"), value: fmt(actual), icon: TrendingUp },
          { label: t("budget.variance"), value: fmt(variance), icon: variance > 0 ? AlertTriangle : CheckCircle2, color: variance > 0 ? "text-destructive" : "text-primary" },
          { label: t("budget.execution"), value: `${projection.toFixed(0)}%`, icon: TrendingUp, color: projection > 100 ? "text-destructive" : "text-primary" },
        ].map((m) => (
          <Card key={m.label}>
              ...
              <div key={b.id} className="rounded-md border border-border">
                <div className="flex items-center gap-2 p-3">
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <div className="text-xs text-muted-foreground">{b.category} · {b.date} · {t("budget.subitems_count", { count: b.subItems.length })}</div>
                  </div>
                  <div className="text-right">
                    <div className="text-xs text-muted-foreground">{t("budget.planned")}</div>
                    <div className="text-sm tabular-nums">{fmt(b.planned)}</div>
                  </div>
                  <div className="text-right">
                    <div className="text-xs text-muted-foreground">{t("budget.actual")}</div>
                    <div className="text-sm tabular-nums">{fmt(real)}</div>
                  </div>
                  <div className="text-right w-24">
                    <div className="text-xs text-muted-foreground">{t("budget.variance")}</div>
                ...
                {isOpen && (
                  <div className="border-t border-border p-3 space-y-2 bg-muted/20">
                    {b.subItems.length === 0 ? (
                      <p className="text-xs text-muted-foreground">{t("budget.no_subitems")}</p>
                    ) : (
                    ...
                    <Button variant="outline" size="sm" onClick={() => setSubDialog({ budgetId: b.id })}>
                      <Plus className="h-3.5 w-3.5" /> {t("budget.add_subitem")}
                    </Button>
                  </div>
                )}
              </div>
...
function BudgetDialog({ item, onSave, onClose }: {
  ...
  return (
    <Dialog open onOpenChange={(o) => !o && onClose()}>
      <DialogContent>
        <DialogHeader><DialogTitle>{item ? t("budget.dialog_edit") : t("budget.dialog_new")}</DialogTitle></DialogHeader>
        <div className="space-y-3">
          <div className="grid grid-cols-2 gap-3">
            <div><Label>{t("budget.category")}</Label><Input value={form.category} onChange={(e) => setForm({ ...form, category: e.target.value })} /></div>
            <div><Label>{t("budget.date")}</Label><Input type="date" value={form.date} onChange={(e) => setForm({ ...form, date: e.target.value })} /></div>
          </div>
          <div><Label>{t("budget.description")}</Label><Input value={form.description} onChange={(e) => setForm({ ...form, description: e.target.value })} /></div>
          <div>
            <Label>{t("budget.planned_amount")}</Label>
            ...
            <p className="text-xs text-muted-foreground mt-1">{t("budget.planned_hint")}</p>
          </div>
        </div>
        <DialogFooter>
          <Button variant="outline" onClick={onClose}>{t("budget.cancel")}</Button>
          <Button onClick={() => form.category && onSave(form)}>{item ? t("budget.save") : t("budget.create")}</Button>
        </DialogFooter>
      </DialogContent>
    </Dialog>
  );
...
function SubItemDialog({ sub, onSave, onClose }: {
  ...
  return (
    <Dialog open onOpenChange={(o) => !o && onClose()}>
      <DialogContent>
        <DialogHeader><DialogTitle>{sub ? t("budget.sub_dialog_edit") : t("budget.sub_dialog_new")}</DialogTitle></DialogHeader>
        <div className="space-y-3">
          <div><Label>{t("budget.expense_desc")}</Label><Input value={form.description} onChange={(e) => setForm({ ...form, description: e.target.value })} /></div>
          <div className="grid grid-cols-2 gap-3">
            <div>
          ...
          <p className="text-xs text-muted-foreground">{t("budget.sub_hint")}</p>
        </div>
        <DialogFooter>
          <Button variant="outline" onClick={onClose}>{t("budget.cancel")}</Button>
          <Button onClick={() => form.description && onSave(form)}>{sub ? t("budget.save") : t("budget.add")}</Button>
        </DialogFooter>
      </DialogContent>
    </Dialog>
  );
